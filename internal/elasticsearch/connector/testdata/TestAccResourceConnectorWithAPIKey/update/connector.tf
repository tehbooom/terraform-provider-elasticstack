...
locals {
  updated_raw_key_id = element(split("/", elasticstack_elasticsearch_security_api_key.updated.key_id), length(split("/", elasticstack_elasticsearch_security_api_key.updated.key_id)) - 1)
}
...
resource "elasticstack_elasticsearch_search_connector" "test" {
  ...
  api_key_id   = local.updated_raw_key_id
}
